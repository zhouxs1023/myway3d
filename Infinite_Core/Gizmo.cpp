...
namespace Infinite {
	...
	Gizmo::Gizmo()
		: OnInit(xEvent::OnInit, this, &Gizmo::Init)
		, OnShutdown(xEvent::OnShutdown, this, &Gizmo::Shutdown)
		, OnRender(RenderEvent::OnAfterRender, this, &Gizmo::Render)
		, mPicked(false)
		, mPickedAxis(-1)
	{
		mPicked = false;
	...
	void Gizmo::Shutdown(Event * sender)
	{
		delete[] mVertex_Move;
		delete[] mIndex_Move;
		delete mRender_Move;
	...
	void Gizmo::Render(Event * sender)
	{
		int op = Editor::Instance()->GetOperator();

		if (op == eOP_Move)
			_renderMove();
		else if (op == eOP_Rotate)
			_renderRotate();
		else if (op == eOP_Scale)
			_renderScale();
	}
	...
	void Gizmo::_renderMove()
	{
		if (Editor::Instance()->GetSelectedShapeSize() == 0 ||
			Editor::Instance()->GetSelectedShapeSize() > 1)
			return ;
	...
	void Gizmo::_update_Move()
	{
		...
		Point2f pt = Point2f(mMouseX, mMouseY);
	...
	float Gizmo::_getObjSize(Shape * obj)
	{
