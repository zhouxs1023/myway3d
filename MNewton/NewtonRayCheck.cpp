...
namespace Myway {
	...
	namespace Newton {
		...
		void tRayCheck::Do(const Vec3 & start, const Vec3 & end)
		{
			NewtonWorldRayCast(tWorld::Instance()->_getNewtonWorld(), &start.x, &end.x, newtonRaycastFilter, this, newtonRaycastPreFilter);
		}
