...
#include "MWShadowRenderUniform.h"
...
ShadowRenderUniform::ShadowRenderUniform()
: m_distance(50.0f),
  m_near_clip(1.0f),
  mar_clip(10000.0f),
  m_offset(200.0f)
...
bool ShadowRenderUniform::UpdateCamera(Camera * pShadowCamera, int direction, Light * light, Camera * pSceneCamera)
{
...
void ShadowRenderUniform::RenderShadow(Camera * cam, Light * light)
{
    profile_code();
...
void ShadowRenderUniform::RenderShadowCasterQueue()
{
    RenderSystem * render = RenderSystem::Instance().InstancePtr();
...
void ShadowRenderUniform::RenderShadowReceiverQueue()
{
...
void ShadowRenderUniform::UpdateShadowCasterQueue(Camera * pSceneCamera)
{
...
void ShadowRenderUniform::UpdateShadowReceiverQueue()
{
    RenderQueue * rq = RenderQueue::InstancePtr();
...
void ShadowRenderUniform::SetDistance(float dist)
{
    m_distance = dist;
}

void ShadowRenderUniform::SetNearClip(float clip)
{
    m_near_clip = clip;
}

void ShadowRenderUniform::SetFarClip(float clip)
{
    mar_clip =clip;
}

void ShadowRenderUniform::SetOffset(float offset)
{
    m_offset = offset;
}

float ShadowRenderUniform::GetDistance() const
{
    return m_distance;
}

float ShadowRenderUniform::GetNearClip() const
{
    return m_near_clip;
}

float ShadowRenderUniform::GetFarClip() const
{
    return mar_clip;
}

float ShadowRenderUniform::GetOffset() const
{
    return m_offset;
}