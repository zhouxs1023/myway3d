...
{
	return (1.0 - Math::Cos(linearValue * Math::PI)) * 0.5;
}
...
{
	return source*(1.0 - timeValue) + (timeValue*target);
}
...
inline T CosineInterpolate(const T& source, const T& target, const MReal timeValue)
{
	...
	return source*(1.0 - weight) + (weight*target);
}
...
{
	return (1-u-v)*pointA + u*pointB + v*pointC;
}
