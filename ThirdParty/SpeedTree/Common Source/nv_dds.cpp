...
#pragma warning (disable : 4702) // map<> use makes STL use unreachable code
...
#include <assert.h>
#include <map>
#define DDS_DEFINITION
#include "nv_dds.h"

using namespace std;
using namespace nv_dds;

static map<string, GLuint> mMapCache;

///////////////////////////////////////////////////////////////////////////////
// IDV-added Global Functions
//
// LoadDDS takes a filename and returns a texture object

GLuint LoadDDS(const char* pFilename, bool bCompress = true, bool bReport = true)
{
    GLuint uiTextureIndex = mMapCache[string(pFilename)];
	if (!uiTextureIndex)
	{
		if (bCompress && !glCompressedTexImage2D)
		{
			fprintf(stderr, "\bWarning:\b OpenGL image compression not supported, skipping [%s]\n", pFilename);
		}
		else
		{
			FILE* pExists = fopen(pFilename, "r");
			if (!pExists)
			{
				fprintf(stderr, "Warning: cannot find texture file [%s]\n", pFilename);
				return 0;
			}
			fclose(pExists);

			CDDSImage cImage;
			(void) cImage.load(pFilename);

			glGenTextures(1, &uiTextureIndex);
			glEnable(GL_TEXTURE_2D);
			...
			if (!bCompress)
			{
				glTexImage2D(GL_TEXTURE_2D, 0, cImage.get_components( ), cImage.get_width( ), 
					cImage.get_height(), 0, cImage.get_format(), GL_UNSIGNED_BYTE, cImage);

				for (int i = 0; i < cImage.get_num_mipmaps( ); i++)
					glTexImage2D(GL_TEXTURE_2D, i + 1, cImage.get_components( ), 
						cImage.get_mipmap(i).get_width( ), cImage.get_mipmap(i).get_height( ), 
						0, cImage.get_format( ), GL_UNSIGNED_BYTE, cImage.get_mipmap(i));
			}
			else
			{
				if (glCompressedTexImage2D)
				{
					glCompressedTexImage2D(GL_TEXTURE_2D, 0, cImage.get_format( ), 
						 cImage.get_width( ), cImage.get_height( ), 0, cImage.get_size( ), 
						 cImage);

					for (int i = 0; i < cImage.get_num_mipmaps(); i++)
						glCompressedTexImage2D(GL_TEXTURE_2D, i + 1, cImage.get_format( ), 
							cImage.get_mipmap(i).get_width( ), cImage.get_mipmap(i).get_height( ), 0, 
							cImage.get_mipmap(i).get_size( ), cImage.get_mipmap(i));
				}
			}
		}
		...
		mMapCache[string(pFilename)] = uiTextureIndex;
	}
...
void DeleteDDSMapCache(void)
{
    for (map<string, GLuint>::iterator iMap = mMapCache.begin( ); iMap != mMapCache.end( ); ++iMap)
		glDeleteTextures(1, &iMap->second);
}
...
bool CDDSImage::load(string filename, bool flipImage)
{
    ...
    // read in DDS header
	(void) fread(&ddsh, sizeof(ddsh), 1, fp);
    ...
    for (int n = 0; n < (cubemap ? 6 : 1); n++)
    {
        ...
        for (int i = 0; i < numMipmaps && (w || h); i++)
        {
            ...
            (void) fread(mipmap, 1, mipmap.size, fp);
            
            if (flipImage)
            {
                flip(mipmap, mipmap.width, mipmap.height, mipmap.depth, mipmap.size);
            }
...
///////////////////////////////////////////////////////////////////////////////
// copy constructor
CTexture::CTexture(const CTexture &texCopy)
  : CSurface(texCopy)
{
    for (unsigned int i = 0; i < texCopy.mipmaps.size(); i++)
        mipmaps.push_back(texCopy.mipmaps[i]);
}
...
///////////////////////////////////////////////////////////////////////////////
// copy constructor
CSurface::CSurface(const CSurface &surCopy)
  : width(0),
    height(0),
    depth(0),
    size(0),
    pixels(NULL)
{
    ...
    if (surCopy.pixels)
    {
        size = surCopy.size;
        width = surCopy.width;
        height = surCopy.height;
        depth = surCopy.depth;
        pixels = new char[size];
        memcpy(pixels, surCopy.pixels, surCopy.size);
    }
}
